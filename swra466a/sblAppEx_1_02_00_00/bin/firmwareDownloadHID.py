...
def subprocess_call(strOpen, functionSelect):
	...
	# Check for timeout i.e. sending autobaud failed to get response
	if strReadStdout.find("Timed") != -1:
		print "Error connecting to Dongle."
		quit()

	# SBL Read Function
	if functionSelect == SBLFunction.READ:
		if strReadStdout.find("Error") != -1:
			print "Error reading from firmware."
			quit()
...
def copy_flash():
	...
	for i in range(CONST_NUM_FLASH_PAGES):
		...
		sys.stdout.flush()
		lstFlashPages.append(subprocess_call("sudo ./firmwareDownloadHID -s -r{} -n4096".format(i*4096), SBLFunction.READ))
...
def get_addr_location():
	strAddrOfBTA = subprocess_call("sudo ./firmwareDownloadHID -s -f0x48b4b0", SBLFunction.FIND)
...
def parse_cmd_args():
	...
	parser = argparse.ArgumentParser(prog="python firmwareDownloadHID.py",
		formatter_class=argparse.RawTextHelpFormatter,
		description="Python program to change Bluetooth Address of Dongle using the SBL library")
	...
	parser.add_argument("-f", "--find", metavar="SEARCH_STR", help="Search for string (for hex use '0x') in Dongle firmware")
	...
	parser.add_argument("-a", "--address", action="store_true", help="Read Bluetooth Address currently stored in Dongle")
	parser.add_argument("-n", "--new-address", metavar="BLUETOOTH_ADDR", help="Set new Bluetooth Adress in Dongle in hex")
	parser.add_argument("file", nargs="?", metavar="FILE",
		help="File path for bin input of Dongle flash\nWARNING: WILL REWRITE FULL FLASH")
...
if __name__ == "__main__":
	...
	if readCurrentAddr:
		...
		strOldBTA = subprocess_call("sudo ./firmwareDownloadHID -s -r{} -n6".format(strAddrOfBTA), SBLFunction.READ)
	...
	if strBTA:
		...
		subprocess_call("sudo ./firmwareDownloadHID -w{} {}".format(strAddrOfBTA, strRealBTA), SBLFunction.WRITE)
	...
	if strBytesToFind:
		...
		strFound = subprocess_call("sudo ./firmwareDownloadHID -s -f{}".format(strBytesToFind), SBLFunction.FIND)
	...
	if lstReadData:
		...
		strData = subprocess_call(
			"sudo ./firmwareDownloadHID -r{} -n{}".format(strReadAddr, nReadLength),
			SBLFunction.READ)
	...
	if lstWriteData:
		...
		subprocess_call("sudo ./firmwareDownloadHID -s -w{} {}".format(strWriteAddr, strBytesToWrite), SBLFunction.WRITE)
		print "Success."
		quit()

	if fWriteFlash:
		if not bNoBackup: backup_flash()
		subprocess_call("sudo ./firmwareDownloadHID {}".format(fWriteFlash), SBLFunction.REWRITE_FLASH)
